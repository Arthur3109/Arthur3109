# Olá, eu sou o Arthur Emmerich Lamas! 

Estudante de **Informática para Internet na ETEC Praia Grande** e aspirante a **Desenvolvedor Front-End**. Sou apaixonado por transformar ideias em interfaces funcionais e responsivas, focando sempre na experiência do usuário.

---

### 🚀 Sobre Mim
-  Atualmente cursando o Técnico em Informática (Extensão ETEC).
-  Focado em dominar o ecossistema Web (HTML, CSS e JavaScript).
-  Interessado em UI/UX Design.
-  Em constante aprendizado e busca por evolução técnica.

---

###  Tecnologias e Ferramentas
![HTML5](https://img.shields.io/badge/html5-%23E34F26.svg?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/css3-%231572B6.svg?style=for-the-badge&logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/javascript-%23F7DF1E.svg?style=for-the-badge&logo=javascript&logoColor=black)
![Git](https://img.shields.io/badge/git-%23F05032.svg?style=for-the-badge&logo=git&logoColor=white)

---

###  Projeto em Destaque
#### **Landing Page π-Tágoras**
Projeto interdisciplinar finalista, onde fui responsável por todo o desenvolvimento do **Front-End** e **UI Design**.
- **Destaques:** Implementação de sistema de acessibilidade (ajuste de contraste e fontes).
- **Tech Stack:** HTML5, CSS3 e JavaScript.
- [ Ver Repositório do Projeto](https://arthur3109.github.io/Site-Institucional/)

---

### 📫 Como me encontrar
[![Instagram](https://img.shields.io/badge/instagram-%23E4405F.svg?style=for-the-badge&logo=instagram&logoColor=white)](https://instagram.com/arthur.emmerich01)
[![Gmail](https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:arthurlamas09@gmail.com)

---
*Apelido: DevAEL*
